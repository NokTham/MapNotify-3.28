...
namespace MapNotify_3_28
{
    partial class MapNotify_3_28 : BaseSettingsPlugin<MapNotifySettings>
    {
        ...
        {
            // Enforce: A mod cannot be both Good and Bad
            ...
            // If the color is still the default white, assign a default based on good/bad
            if (mod.Color == new nuVector4(1, 1, 1, 1)) // Check for default white
            {
                if (fileName == "GoodMods.txt")
                {
                    mod.Color = new nuVector4(0.4f, 1f, 0.4f, 1f); // Green
                }
                else if (fileName == "BadMods.txt")
                {
                    mod.Color = new nuVector4(1f, 0.4f, 0.4f, 1f); // Red
                }
            }
        ...
        private void DeleteModFromConfig(string rawName)
        {
            ...
            {
                LogError($"Could not find {rawName} in GoodMods.txt or BadMods.txt", 5);
            }
        }
